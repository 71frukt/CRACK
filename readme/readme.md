...
```
То есть если PSWD_MAX_LEN = 5d, то введенный пароль `$00000-a` пройдёт верификацию. Числа после '\$' не будут влиять на хеш, а последовательность байт `A0C4`, затирающая значение верного хеш-кода, соответствует хеш-коду пустого пароля (начинающегося с '$').
